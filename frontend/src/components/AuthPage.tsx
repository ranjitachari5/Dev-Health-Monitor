...
export const AuthPage: React.FC<AuthPageProps> = ({ onLogin, onBack }) => {
  ...
  // Add an effect to help user see changes immediately by clearing on hot reload (optional debugging logic)
  React.useEffect(() => {
    // If they manually navigate to "Sign In", let's clear old state so they can test it again
  ...
  return (
    <div className="relative w-full h-full min-h-screen flex items-center justify-center p-6 font-mono text-blue-500 overflow-hidden bg-[#0A0E17]">
      ...
      {/* Toast Notification */}
      {showToast && (
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-black border border-[#3b82f6] text-[#3b82f6] px-6 py-3 rounded shadow-[0_0_15px_rgba(59,130,246,0.3)] animate-fade-in-up flex items-center gap-3">
